...
// ÷ифры - всегда в верхнем регистре, префикс 0x - в нижнем
inline
...
bool isNumericLanguageTag(const std::string &langTag)
{
    ...
    if (langTag.size()>2 && langTag[0]=='0' && (langTag[1]=='x' || langTag[1]=='X')) // „исло с префиксом 0x
        return true;
    ...
    // aa, ae - это строковые тэги локалей, но похожи на числа. Ќо таких коротких чисел не должно быть
...
//TODO: !!! Ёто не совсем безопасно, но если считать, что все настройки будут в однопотоке при запуске проги и потом не мен€ютс€, то норм
// ¬ообще подразумеваетс€, что они никогда не будут мен€тьс€, что и так всЄ хорошо настроено
                 ...
                 )
{
    ...
    for(auto i=0u; i!=localesTotal; ++i)
    {
        ...
        if (tagToIndexMap)
        {
            ...
            if (pLocales[i].langId!=0x1000)
            {
                // “акже хотим, чтобы мапились виндовые локали вида 0xNNNN/0XNNNN/0xNNN/0XNNN/NNNN/NNN в строковом представлении
...
const LocaleInfo* getLocaleInfo( std::string langTag, bool neutralAllowed = false )
{
    ...
    // ƒл€ числовых langTag'ов просто - если найден, то там то, что нужно
    ...
    /* ≈сли находим по language tag, что ему нет ассоциированных Language ID, то вырезаем nlang часть, и ищем по ней.
       ≈сли находим по nlang, что ему нет ассоциированных Language ID, то ищем подход€щий не нейтральный
...
// ISO 3166-1:2020
// Codes for the representation of names of countries and their subdivisions Ч Part 1: Country code
